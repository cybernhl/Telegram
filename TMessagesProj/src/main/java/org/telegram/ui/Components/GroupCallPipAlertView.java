...
public class GroupCallPipAlertView extends LinearLayout implements VoIPService.StateListener, NotificationCenter.NotificationCenterDelegate {
    ...
    public GroupCallPipAlertView(Context context, int account) {
            ...
            public void onInitializeAccessibilityNodeInfo(AccessibilityNodeInfo info) {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                    ...
                    if (service != null && ChatObject.isChannelOrGiga(service.getChat())) {
                        info.addAction(new AccessibilityNodeInfo.AccessibilityAction(AccessibilityNodeInfo.ACTION_CLICK, LocaleController.getString(org.telegram.messenger.utils.LangMultiExtKt.KEY_VOIPCHANNELOPENVOICECHAT, R.string.VoipChannelOpenVoiceChat)));
                    } else {
                        info.addAction(new AccessibilityNodeInfo.AccessibilityAction(AccessibilityNodeInfo.ACTION_CLICK, LocaleController.getString(org.telegram.messenger.utils.LangMultiExtKt.KEY_VOIPGROUPOPENVOICECHAT, R.string.VoipGroupOpenVoiceChat)));
                    }
                }
            }
        };
        ...
        leaveButton.setData(R.drawable.calls_decline, 0xffffffff, 0xFFCE4A4A, 0.3f, false, LocaleController.getString(org.telegram.messenger.utils.LangMultiExtKt.KEY_VOIPGROUPLEAVE, R.string.VoipGroupLeave), false, false);
    ...
    private void updateMembersCount() {
        ...
        if (service != null && service.groupCall != null) {
            ...
            if (!service.isSwitchingStream() && (currentCallState == VoIPService.STATE_WAIT_INIT || currentCallState == VoIPService.STATE_WAIT_INIT_ACK || currentCallState == VoIPService.STATE_CREATING || currentCallState == VoIPService.STATE_RECONNECTING)) {
                subtitleView.setText(LocaleController.getString(org.telegram.messenger.utils.LangMultiExtKt.KEY_VOIPGROUPCONNECTING, R.string. VoipGroupConnecting));
            } else {
    ...
    private void updateButtons(boolean animated) {
        ...
        if (service.mutedByAdmin()) {
            muteButton.setData(R.drawable.calls_unmute, Color.WHITE, ColorUtils.setAlphaComponent(Color.WHITE, (int) (255 * 0.3f)), 0.1f, true, LocaleController.getString(org.telegram.messenger.utils.LangMultiExtKt.KEY_VOIPMUTEDBYADMINSHORT, R.string.VoipMutedByAdminShort), true, animated);
        } else {
            muteButton.setData(R.drawable.calls_unmute, Color.WHITE, ColorUtils.setAlphaComponent(Color.WHITE, (int) (255 * (service.isMicMute() ? 0.3f : 0.15f))), 0.1f, true, service.isMicMute() ? LocaleController.getString(org.telegram.messenger.utils.LangMultiExtKt.KEY_VOIPUNMUTE, R.string.VoipUnmute) : LocaleController.getString("VoipMute", R.string.VoipMute), service.isMicMute(), animated);
        }
        invalidate();
    }
