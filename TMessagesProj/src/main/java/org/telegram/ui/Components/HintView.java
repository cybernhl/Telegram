...
public class HintView extends FrameLayout {
    ...
    public HintView(Context context, int type, boolean topArrow, Theme.ResourcesProvider resourcesProvider) {
        ...
        if (type == TYPE_NOSOUND) {
            textView.setText(LocaleController.getString(org.telegram.messenger.utils.LangMultiExtKt.KEY_AUTOPLAYVIDEOINFO, R.string.AutoplayVideoInfo));
    ...
    public boolean showForMessageCell(ChatMessageCell cell, Object object, int x, int y, boolean animated) {
        ...
        if (currentType == TYPE_NOSOUND) {
            ...
            shownY = y;
            if (count == -1) {
                textView.setText(LocaleController.getString(org.telegram.messenger.utils.LangMultiExtKt.KEY_POLLSELECTOPTION, R.string.PollSelectOption));
            } else {
                if (cell.getMessageObject().isQuiz()) {
                    if (count == 0) {
                        textView.setText(LocaleController.getString(org.telegram.messenger.utils.LangMultiExtKt.KEY_NOVOTESQUIZ, R.string.NoVotesQuiz));
                    } else {
                        textView.setText(LocaleController.formatPluralString("Answer", count));
                    }
                } else {
                    if (count == 0) {
                        textView.setText(LocaleController.getString(org.telegram.messenger.utils.LangMultiExtKt.KEY_NOVOTES, R.string.NoVotes));
                    } else {
            ...
            if (overrideText == null) {
                textView.setText(LocaleController.getString(org.telegram.messenger.utils.LangMultiExtKt.KEY_HIDACCOUNT, R.string.HidAccount));
            } else {
