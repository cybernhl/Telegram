            ...
            if (requestPeerType instanceof TLRPC.TL_requestPeerTypeUser) {
                ...
                if (isChannel) {
                    ...
                    if (requestPeerType.bot_participant != null && requestPeerType.bot_participant) {
                        requirements.add(Requirement.make(AndroidUtilities.replaceTags(LocaleController.getString(org.telegram.messenger.utils.LangMultiExtKt.KEY_PEERREQUIREMENTCHANNELBOTPARTICIPANT, R.string.PeerRequirementChannelBotParticipant))));
                    }
                    if (requestPeerType.creator != null && requestPeerType.creator) {
                        requirements.add(Requirement.make(AndroidUtilities.replaceTags(LocaleController.getString(org.telegram.messenger.utils.LangMultiExtKt.KEY_PEERREQUIREMENTCHANNELCREATORTRUE, R.string.PeerRequirementChannelCreatorTrue))));
                    }
                } else {
                    ...
                    if (requestPeerType.bot_participant != null && requestPeerType.bot_participant) {
                        requirements.add(Requirement.make(AndroidUtilities.replaceTags(LocaleController.getString(org.telegram.messenger.utils.LangMultiExtKt.KEY_PEERREQUIREMENTGROUPBOTPARTICIPANT, R.string.PeerRequirementGroupBotParticipant))));
                    }
                    if (requestPeerType.creator != null && requestPeerType.creator) {
                        requirements.add(Requirement.make(AndroidUtilities.replaceTags(LocaleController.getString(org.telegram.messenger.utils.LangMultiExtKt.KEY_PEERREQUIREMENTGROUPCREATORTRUE, R.string.PeerRequirementGroupCreatorTrue))));
                    }
                }
...
//                checkAdminRights(requestPeerType.bot_admin_rights, isChannel, R.string.PeerRequirementBotRights, R.string.PeerRequirementBotRight);
            ...
            if (!requirements.isEmpty()) {
                ...
                headerCell.setText(LocaleController.getString(org.telegram.messenger.utils.LangMultiExtKt.KEY_PEERREQUIREMENTS, R.string.PeerRequirements));
    ...
    public static CharSequence rightsToString(TLRPC.TL_chatAdminRights rights, boolean isChannel) {
        ...
        if (rights.change_info) {
            array.add(Requirement.make(
                    1,
                    isChannel ?
                            LocaleController.getString(org.telegram.messenger.utils.LangMultiExtKt.KEY_EDITADMINCHANGECHANNELINFO, R.string.EditAdminChangeChannelInfo) :
                            LocaleController.getString(org.telegram.messenger.utils.LangMultiExtKt.KEY_EDITADMINCHANGEGROUPINFO, R.string.EditAdminChangeGroupInfo)
            ));
        }
        if (rights.post_messages && isChannel) {
            array.add(Requirement.make(1, LocaleController.getString(org.telegram.messenger.utils.LangMultiExtKt.KEY_EDITADMINPOSTMESSAGES, R.string.EditAdminPostMessages)));
        }
        if (rights.edit_messages && isChannel) {
            array.add(Requirement.make(1, LocaleController.getString(org.telegram.messenger.utils.LangMultiExtKt.KEY_EDITADMINEDITMESSAGES, R.string.EditAdminEditMessages)));
        }
        if (rights.delete_messages) {
            array.add(Requirement.make(1, isChannel ? LocaleController.getString(org.telegram.messenger.utils.LangMultiExtKt.KEY_EDITADMINDELETEMESSAGES, R.string.EditAdminDeleteMessages) : LocaleController.getString("EditAdminGroupDeleteMessages", R.string.EditAdminGroupDeleteMessages)));
        }
        if (rights.ban_users && !isChannel) {
            array.add(Requirement.make(1, LocaleController.getString(org.telegram.messenger.utils.LangMultiExtKt.KEY_EDITADMINBANUSERS, R.string.EditAdminBanUsers)));
        }
        if (rights.invite_users) {
            array.add(Requirement.make(1, LocaleController.getString(org.telegram.messenger.utils.LangMultiExtKt.KEY_EDITADMINADDUSERS, R.string.EditAdminAddUsers)));
        }
        if (rights.pin_messages && !isChannel) {
            array.add(Requirement.make(1, LocaleController.getString(org.telegram.messenger.utils.LangMultiExtKt.KEY_EDITADMINPINMESSAGES, R.string.EditAdminPinMessages)));
        }
        if (rights.add_admins) {
            array.add(Requirement.make(1, LocaleController.getString(org.telegram.messenger.utils.LangMultiExtKt.KEY_EDITADMINADDADMINS, R.string.EditAdminAddAdmins)));
        }
        if (rights.anonymous && !isChannel) {
            array.add(Requirement.make(1, LocaleController.getString(org.telegram.messenger.utils.LangMultiExtKt.KEY_EDITADMINSENDANONYMOUSLY, R.string.EditAdminSendAnonymously)));
        }
        if (rights.manage_call) {
            array.add(Requirement.make(1, LocaleController.getString(org.telegram.messenger.utils.LangMultiExtKt.KEY_STARTVOIPCHATPERMISSION, R.string.StartVoipChatPermission)));
        }
        if (rights.manage_topics && !isChannel) {
            array.add(Requirement.make(1, LocaleController.getString(org.telegram.messenger.utils.LangMultiExtKt.KEY_MANAGETOPICSPERMISSION, R.string.ManageTopicsPermission)));
        }
    ...
    private void checkAdminRights(TLRPC.TL_chatAdminRights value, boolean isChannel, CharSequence headerText, CharSequence headerSingleText) {
        ...
        if (value.post_messages && isChannel) {
            rights.add(Requirement.make(1, LocaleController.getString(org.telegram.messenger.utils.LangMultiExtKt.KEY_EDITADMINPOSTMESSAGES, R.string.EditAdminPostMessages)));
        }
        if (value.edit_messages && isChannel) {
            rights.add(Requirement.make(1, LocaleController.getString(org.telegram.messenger.utils.LangMultiExtKt.KEY_EDITADMINEDITMESSAGES, R.string.EditAdminEditMessages)));
        }
        if (value.delete_messages) {
            rights.add(Requirement.make(1, isChannel ? LocaleController.getString(org.telegram.messenger.utils.LangMultiExtKt.KEY_EDITADMINDELETEMESSAGES, R.string.EditAdminDeleteMessages) : LocaleController.getString("EditAdminGroupDeleteMessages", R.string.EditAdminGroupDeleteMessages)));
        }
        if (value.ban_users && !isChannel) {
            rights.add(Requirement.make(1, LocaleController.getString(org.telegram.messenger.utils.LangMultiExtKt.KEY_EDITADMINBANUSERS, R.string.EditAdminBanUsers)));
        }
        if (value.invite_users) {
            rights.add(Requirement.make(1, LocaleController.getString(org.telegram.messenger.utils.LangMultiExtKt.KEY_EDITADMINADDUSERS, R.string.EditAdminAddUsers)));
        }
        if (value.pin_messages && !isChannel) {
            rights.add(Requirement.make(1, LocaleController.getString(org.telegram.messenger.utils.LangMultiExtKt.KEY_EDITADMINPINMESSAGES, R.string.EditAdminPinMessages)));
        }
        if (value.add_admins) {
            rights.add(Requirement.make(1, LocaleController.getString(org.telegram.messenger.utils.LangMultiExtKt.KEY_EDITADMINADDADMINS, R.string.EditAdminAddAdmins)));
        }
        if (value.anonymous && !isChannel) {
            rights.add(Requirement.make(1, LocaleController.getString(org.telegram.messenger.utils.LangMultiExtKt.KEY_EDITADMINSENDANONYMOUSLY, R.string.EditAdminSendAnonymously)));
        }
        if (value.manage_call) {
            rights.add(Requirement.make(1, LocaleController.getString(org.telegram.messenger.utils.LangMultiExtKt.KEY_STARTVOIPCHATPERMISSION, R.string.StartVoipChatPermission)));
        }
        if (value.manage_topics && !isChannel) {
            rights.add(Requirement.make(1, LocaleController.getString(org.telegram.messenger.utils.LangMultiExtKt.KEY_MANAGETOPICSPERMISSION, R.string.ManageTopicsPermission)));
        }
