...
class ContactsRemoteViewsFactory implements RemoteViewsService.RemoteViewsFactory {
    ...
    public RemoteViews getViewAt(int position) {
        if (deleted) {
            ...
            rv.setTextViewText(R.id.widget_deleted_text, LocaleController.getString(org.telegram.messenger.utils.LangMultiExtKt.KEY_WIDGETLOGGEDOFF, R.string.WidgetLoggedOff));
            return rv;
        } else if (position >= getCount() - 1) {
            RemoteViews rv = new RemoteViews(mContext.getPackageName(), R.layout.widget_edititem);
            rv.setTextViewText(R.id.widget_edititem_text, LocaleController.getString(org.telegram.messenger.utils.LangMultiExtKt.KEY_TAPTOEDITWIDGETSHORT, R.string.TapToEditWidgetShort));
